...
(\frac{1}{3})_{10} = \boldsymbol{(0.\overline{3})_{10}}$

\item $(0.113)_{F!} = \frac{1}{2!} + \frac{1}{3!} + \frac{3}{4!} =
(\frac{19}{24})_{10} = \boldsymbol{(0.791\overline{6})_{10}}$
...
\item A precisão decimal equivalente é igual a $2^{1 - t} = 2^{1 - 3} = 2^{-2}
= \boldsymbol{2.5 \cdot 10^{-1}}$.

\item $F(2, 3, 0, 7)$ padrão IEEE 754 (1985) com $d_1 \neq 0 = 1$ antes da
vírgula e representado implicitamente antes da vírgula e mais 3 bits
significativos após a vírgula e  polarização $p = +3$, conforme segue:
\begin{align*}
\begin{cases}
0 < e < 7 & \longrightarrow v = (-1)^s \cdot 2^{e-3} \cdot (1.f)_2 \\
e = 0, f \neq 0 & \longrightarrow (-1)^s \cdot 2^{-2} \cdot (0.f)_2 \\
e = 0, f = 0 & \longrightarrow 0 \\
e = 7 & \longrightarrow v \text{ pertence à região de \emph{overflow}}
\end{cases}
\end{align*}
...
\begin{tabular}{*{8}{|c}|}
\hline
$s_1$ & $d_2$ & $d_3$ & $d_4$ & $0$ & $e_1$ & $e_2$ & $e_3$ \\
\hline
\end{tabular}
\begin{alphenum}

\item $NM = (\beta - 1) \cdot \beta^{t - 1} = (2 - 1) \cdot 2^{4 - 1} =
\boldsymbol{8}$
...
\item $(-1)^s \cdot 2^{-2} \cdot 0 = \boldsymbol{0}$
...
\item $2^0 + 2^{-52} \longrightarrow$ precisão de até \textbf{17} dígitos
decimais.

\item $2^{-52} = 10^{1-d} \longrightarrow 1 - d = -52 \cdot log(2)
\longrightarrow d = 1 + 52 \cdot log(2) \approx \boldsymbol{16.65356}$.

\item Imprimindo \verb!double x = 0.1! com 30 casas após à vírgula, observa-se
que existem 17 decimais exatos, portanto a precisão decimal equivalente está
entre \textbf{16 e 17 dígitos}.
